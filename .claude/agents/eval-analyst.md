...
description: >
  ...
  on a slice, agent trajectory, tool call accuracy, judge agreement, groundedness, RAG eval.
tools: Read, Grep, Glob, Bash
skills: evaluation, datasets, agent-evaluation
---
...
**For LLM and agentic systems**, the `agent-evaluation` skill is preloaded and its discipline applies
on top of the rules below:
- **Score the trajectory, not just the final answer.** A correct answer reached by calling the wrong
  tool, crossing a tenant boundary, or taking 14 steps to do 2 is a failure the final answer hides.
  Report tool-selection accuracy, argument correctness, step efficiency, and recovery rate.
- **A judge is a measurement instrument and needs validating.** Report judge-vs-human agreement
  (κ or correlation) whenever you report judge scores; an unvalidated judge score is an unmarked
  estimate. Check for position, verbosity, and self-preference bias.
- **Never report a single run.** LLM outputs are stochastic — n≥3, mean ± sd. A "gain" inside the
  noise band is not a gain (`statistics`).
- **Decompose RAG failures** into retrieval quality, groundedness, answer relevance, and context
  sufficiency. "The answer was wrong" has three different causes with three different fixes.
- **Report quality, cost, and latency together.** A change that improves quality 2% and triples cost
  is a decision, not an improvement.
- Adversarial and safety results are a first-class suite, not an appendix — generated by
  `red-teamer`, scored here, and gated absolutely in CI.
